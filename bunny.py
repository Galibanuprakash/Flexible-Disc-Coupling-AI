...
if st.button("Find Best Couplings") and df is not None:
    if power is not None:
        # Apply filtering based on user input
        df_filtered = df.iloc[(df["Speed (RPM)"] - speed).abs().argsort()[:5]]
        df_filtered = df_filtered.iloc[(df_filtered["Power (kW)"] - power).abs().argsort()[:5]]
        
        if not df_filtered.empty:
            st.success("Best Matching Couplings:")
            st.write(df_filtered.to_dict(orient="records"))
        else:
            st.error("No suitable couplings found. Try adjusting the parameters.")
    else:
