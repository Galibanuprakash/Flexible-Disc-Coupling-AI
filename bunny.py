...
if st.button("Find Best Coupling") and df is not None:
    if power is not None:
        ...
        df_sorted = df.sort_values(by=["Speed Difference", "Power Difference"]).head(3)  # Get top 3 closest matches
        
        if not df_sorted.empty:
            st.success("Best Matching Couplings:")
            st.write(df_sorted.drop(columns=["Speed Difference", "Power Difference"]).to_dict(orient="records"))
        else:
