...
if st.button("Find Best Coupling") and df is not None:
    if power is not None:
        # Find closest matches for both speed and power together
        df["Combined Difference"] = abs(df["Speed (RPM)"] - speed) + abs(df["Power (kW)"] - power)
        df_sorted = df.sort_values(by=["Combined Difference"]).head(5)  # Get top 3 closest matches
        
        if not df_sorted.empty:
            st.success("Best Matching Couplings:")
            st.write(df_sorted.drop(columns=["Combined Difference"]).to_dict(orient="records"))
        else:
